# Medha Singh
#Employee Attrition
# Analyze data using 5 different models to find trends in employee attribution.
...
setwd("C:/Users/ual-laptop/Documents/GitHub/Datamining/EmployeeAttrition")
...
# Classify
originalEmployeeRetention <- originalEmployeeRetention %>%
    mutate(Education = ifelse(
        Education == 1, "Below College", ifelse(
            Education == 2, "College", ifelse(
                Education == 3, "Bachelor", ifelse(
                    Education == 4, "Master", "Doctor")))))

# Generate histogram for gender
histogramEmployeeRetentionGender <- ggplot(data = originalEmployeeRetention,
                                           aes(x = Gender,
                                               fill = factor(Attrition)))

# Using geom_histogram function to modify historgram
histogramEmployeeRetentionGender + geom_histogram(
    binwidth = 5,
    stat="count") +
    ggtitle("Attrition by gender") + 
    scale_fill_manual(values = c("#ff68d4", "#349bff"))

# Generate density plot for age
densityEmployeeRetentionAge <- ggplot(data = originalEmployeeRetention,
                                              aes(x = Age,
                                                  fill = factor(Attrition)))

# Using geom_density function to modify density
densityEmployeeRetentionAge + 
    geom_density(alpha = 0.5) + 
    scale_fill_manual(values = c("#ff68d4", "#349bff"))

# Generate histogram for Department
histogramEmployeeRetentionDepartment <- ggplot(data = originalEmployeeRetention,
                                               aes(x = Department,
                                                   fill = factor(Attrition)))

# Using geom_histogram function to modify historgram
histogramEmployeeRetentionDepartment + 
...
displayAllHistograms(originalEmployeeRetention)
...
employeeRetentionTrainingBinned <- aggregate(employeeRetentionTrainingSmoted,
                 by = list(cut(employeeRetentionTrainingSmoted$RelationshipSatisfaction,
                             breaks = 5)),
                 mean)
print(employeeRetentionTrainingBinned)

employeeRetentionTrainingBinned <- employeeRetentionTrainingSmoted %>%
    mutate(Age = ifelse(
        Age <= 3.13, 3.021998, ifelse(
            Age <= 3.37, 3.278981, ifelse(
                Age <= 3.61, 3.500391, ifelse(
                    Age <= 3.85, 3.728802, 3.958751)))))

employeeRetentionTrainingBinned <- employeeRetentionTrainingBinned %>%
    mutate(DistanceFromHome = ifelse(
        DistanceFromHome <= 0.673, 0.1033539, ifelse(
            DistanceFromHome <= 1.35, 0.8824513, ifelse(
                DistanceFromHome <= 2.02, 1.7004085, ifelse(
                    DistanceFromHome <= 2.69, 2.2941604, 3.0653299)))))

employeeRetentionTrainingBinned <- employeeRetentionTrainingBinned %>%
    mutate(RelationshipSatisfaction = ifelse(
        RelationshipSatisfaction <= 0.277, 0.03358094, ifelse(
            RelationshipSatisfaction <= 0.555, 0.40797426, ifelse(
                RelationshipSatisfaction <= 0.832, 0.70171114, ifelse(
                    RelationshipSatisfaction <= 1.11, 1.0666331, 1.33775840)))))

# Generate the Naive Bayes model
employeeRetentionBayesModel <- naiveBayes(
    formula = Attrition ~ .,
    data = employeeRetentionTrainingBinned,
    laplace = 1)
